...
export async function list(storeDir: string): Promise<string[]> {
  const stdout = await run(["--store-dir", storeDir, "list", "--json"]);
  return JSON.parse(stdout) as string[];
}

export async function show(entry: string, storeDir: string): Promise<string> {
  return run(["--store-dir", storeDir, "show", entry]);
}

export async function otp(entry: string, storeDir: string): Promise<OtpResult> {
